...
export function Gallery({ images, alt }: { images: readonly ListingImage[]; alt: string }) {
  if (images.length === 0) {
    return <div className="flex aspect-[4/3] items-center justify-center rounded-lg bg-neutral-200 text-neutral-700">Sin fotos</div>;
  }
  return (
    <div>
      <ul className="flex snap-x snap-mandatory gap-2 overflow-x-auto" aria-label={`Fotos (${images.length})`}>
        {images.map((img, i) => (
          <li key={`${img.url}-${i}`} className="relative w-full shrink-0 snap-center md:w-[48%]">
            <Image
              src={img.url}
              ...
              priority={i === 0}
              className="aspect-[4/3] w-full rounded-lg bg-neutral-200 object-cover"
            />
            {img.isCatalogPhoto ? (
